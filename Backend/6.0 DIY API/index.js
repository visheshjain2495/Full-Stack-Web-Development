...
app.get('/random', (req,res) => {
  res.json(jokes[Math.round(Math.random() * jokes.length)]);
});

//2. GET a specific joke
app.get('/jokes/:id', (req, res) => {
  const id = parseInt(req.params.id);
  res.json(jokes[jokes.findIndex((joke)=> joke.id === id)]);
});
...
app.put('/jokes/:id', (req,res) => {
  ...
  try{
    Object.assign(jokes.find((joke) => joke.id === id), newValues);
...
app.patch('/jokes/:id', (req,res) => {
  const id = parseInt(req.params.id);
  try{
    const initialJoke = jokes.find((joke) => joke.id === id);
    const updatedJoke = {
      jokeText: req.body.text || initialJoke.jokeText,
      jokeType: req.body.type || initialJoke.jokeType
    };
    Object.assign(jokes.find((joke) => joke.id === id), updatedJoke);
...
app.delete('/all', (req, res) => {
  ...
  if (masterKey === req.query.key){
    ...
    res.json({error: "incorrect master key."});
  }
});
